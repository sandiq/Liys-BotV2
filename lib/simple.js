const {
  ...
  exports.makeWASocket = (connectionOptions, options = {}) => {
    let ptz = (global.opts["legacy"] ? makeWALegacySocket: makeWASocket)(
      connectionOptions,
    );
    ...
    ptz.ev.on(
      ...
      async function updateParticipantsToDb( {
    ...
    ptz.ev.on("chats.upsert",
      async function chatsUpsertPushToDb(chatsUpsert) {
        console.log({
          chatsUpsert
        });
        const {
        ...
        if (isGroup) {
          ...
          const groups =
          (await ptz.groupFetchAllParticipating().catch((_) => ({}))) || {};
    ...
    ptz.ev.on(
      "presence.update",
      async function presenceUpdatePushToDb( {
        id, presences
      }) {
    ...
    ptz.appendTextMessage = async (m,
      ...
      messages.key.fromMe = areJidsSameUser(m.sender,
        ptz.user.id);
    ...
    */
    (ptz.sendFile = async (
      ...
      let {
        res, data: file, filename: pathFile
      } = type;
    ...
    ptz.sendImageAsSticker = async (jid,
      path,
      quoted,
      options = {}) => {
      let buff = Buffer.isBuffer(path)
      ? path: /^data:.*?\/.*?;base64,/i.test(path)
      ? Buffer.from(path.split`,`[1],
        "base64"): /^https?:\/\//.test(path)
      ? await (await fetch(path)).buffer(): fs.existsSync(path)
    ...
    ptz.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
      ...
      ? path: /^data:.*?\/.*?;base64,/i.test(path)
      ? Buffer.from(path.split`,`[1], "base64"): /^https?:\/\//.test(path)
      ? await await fetchBuffer(path): fs.existsSync(path)
      ? fs.readFileSync(path): Buffer.alloc(0);
      ...
      return ptz.sendMessage(
        jid,
        {
          ...
          contacts: {
            ...
            displayName:
            (contacts.length >= 2
              ? `${contacts.length} kontak`: contacts[0].displayName) || null,
            contacts,
          },
        },
        {
...
END:VCARD`.trim()
    ...
    ptz.sendList = async (jid, name, array, quoted, json) => {
      ...
      if (json.url) {
        let file = await ptz.getFile(json.url, true)
              ...
              viewOnceMessage: {
                message: {
                  ...
                  interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    contextInfo: {
                      mentionedJid: [...ptz.parseMention(json.body)],
                      ...(quoted ? {
    ...
    ptz.sendButton = async (jid, array, quoted, json) => {
      const result = [];
    ...
    */
    ptz.cMod = (
      ...
      let msg = isEphemeral
      ? copy.message.ephemeralMessage.message: copy.message;
    ...
    */
    ...
    ptz.downloadAndSaveMediaMessage = async (
      ...
      ? message.mtype.replace(/Message/gi, ""): mime.split("/")[0];
    ...
    */
    ...
    ptz.insertAllGroup = async () => {
      const groups =
      (await ptz.groupFetchAllParticipating().catch((_) => null)) || {};
      for (const group in groups)
        ptz.chats[group] = {
        ...(ptz.chats[group] || {}),
        id: group,
      ...
      for (const message of m) {
        try {
          ...
          if (
            ...
            ptz.processMessageStubType(message).catch(console.error);
          ...
          if (isGroup) {
            if (!chats.subject || !chats.metadata) {
              metadata =
              (await ptz.groupMetadata(chat).catch((_) => ({}))) || {};
              if (!chats.subject) chats.subject = metadata.subject || "";
              if (!chats.metadata) chats.metadata = metadata;
            }
            sender = ptz.decodeJid(
              (message.key?.fromMe && ptz.user.id) ||
              message.participant ||
          ...
          const fromMe =
          message.key.fromMe || areJidsSameUser(sender || chat, ptz.user.id);
          if (
   ...
   */
    ptz.getFile = async (path) => {
      ...
      ? Buffer.from(path.split`,`[1], "base64"): /^https?:\/\//.test(path)
      ? await (res = await fetch(path)).buffer(): fs.existsSync(path)
      ? fs.readFileSync(path): typeof path === "string"
      ? path: Buffer.alloc(0);
      if (!Buffer.isBuffer(data)) throw new TypeError("Result is not a buffer");
  ...
  /**
  * Serialize Message
  * @param {ReturnType<typeof makeWASocket>} ptz
  * @param {proto.WebMessageInfo} m
  * @param {Boolean} store
  */
  exports.smsg = (ptz, m, store) => {
    ...
    if (m.message) {
      ...
      if (m.mtype == "protocolMessage" && m.msg.key) {
        ...
        m.msg.key.fromMe =
        ptz.decodeJid(m.msg.key.participant) === ptz.decodeJid(ptz.user.id);
        if (
          !m.msg.key.fromMe &&
          m.msg.key.remoteJid === ptz.decodeJid(ptz.user.id)
        )
          m.msg.key.remoteJid = m.sender;
      }
      m.text = (m.type === 'conversation') ?  m.message.conversation : (m.type === 'interactiveMessage' || m.type === 'interactiveResponseMessage') ?  JSON.parse(m.msg.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson).id : (m.type == 'imageMessage') ? m.msg.imageMessage.caption : (m.type == 'videoMessage') ? m.msg.videoMessage.caption : (m.type == 'extendedTextMessage') ? m.msg.extendedTextMessage.text : (m.type == 'buttonsResponseMessage') ? m.msg.buttonsResponseMessage.selectedButtonId : (m.type == 'listResponseMessage') ? m.msg.listResponseMessage.singleSelectReply.selectedRowId : (m.type == 'templateButtonReplyMessage') ? m.msg.templateButtonReplyMessage.selectedId : (m.type === 'viewOnceMessageV2') ? m.msg.message.imageMessage ? m.msg.message.imageMessage.caption : m.msg.message.videoMessage.caption : typeof m.msg === "string" ? m.msg : typeof m.msg.text === "string" ? m.msg.text : ""
      m.mentionedJid =
      (m.msg?.contextInfo?.mentionedJid?.length &&
        m.msg.contextInfo.mentionedJid) ||
      [];
      let quoted = (m.quoted = m.msg?.contextInfo?.quotedMessage
        ? m.msg.contextInfo.quotedMessage: null);
      if (m.quoted) {
        ...
        */
        ...
        ptz.reply(chatId ? chatId: m.chat, text, vM, options);
        ...
        */
        ...
        ptz.forwardMessage(jid, vM, forceForward);
        ...
        */
        m.quoted.cMod = (
          ...
          options = {},
        ) => ptz.cMod(jid, vM, text, sender, options);

        /**
        * Delete quoted message
        */
        m.quoted.delete = () =>
        ptz.sendMessage(m.quoted.chat, {
          delete: vM.key
        });
      }
    }
    ...
    ptz.downloadM(m.msg, m.mtype.replace(/message/i, ""), saveToFile);
    /**
    ...
    */
    ...
    ptz.cMod(jid, m, text, sender, options);

    /**
    * Delete this message
    */
    m.delete = () => ptz.sendMessage(m.chat, {
      delete: m.key
    });

    try {
      if (m.msg && m.mtype == "protocolMessage")
        ptz.ev.emit("message.delete", m.msg.key);
    } catch (e) {
      console.error(e);
    }
    return m;
