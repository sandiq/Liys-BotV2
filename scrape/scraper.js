...
async function ytPlayMp4(query) {
  return new Promise((resolve, reject) => {
    try {
      const search = yts(query).then((data) => {
        const url = [];
  ...
  return new Promise((resolve, reject) => {
    axios
      ...
      .then(({ data }) => {
        ...
        let title = $("title")
          .text()
          .replace("Bahasa Indonesia - MyNimeku", "")
          .trim();
        let result = [];
...
function otakudesu(judul) {
  return new Promise(async (resolve, reject) => {
    axios
      .get("https://otakudesu.moe/?s=" + judul + "&post_type=anime")
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const result = {};
        let limk = $(
...
async function fetchData(endpoint, params) {
  try {
    const response = await (
      await axios.post("https://litensi.id/api/" + endpoint, params)
    ).data;
