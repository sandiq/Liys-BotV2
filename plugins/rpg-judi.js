...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  ...
  if (m.chat in conn.judi)
    return m.reply(
      "Masih ada yang melakukan judi disini, tunggu sampai selesai!!",
    );
  ...
  try {
    ...
    if (args.length < 1)
      return conn.reply(
        m.chat,
        usedPrefix + "judi <jumlah>\n " + usedPrefix + "judi 1000",
        m,
      );
    if (global.db.data.users[m.sender].money >= count * 1) {
      global.db.data.users[m.sender].money -= count * 1;
      await m.reply(
        "*Ayo judi klo menang dapat banyak uang!!, kalau gk percaya gpp*",
      ); //Kwkwwkkwlwlw
      if (Aku > Kamu) {
        conn.reply(
          m.chat,
      ...
      conn.reply(
        m.chat,
        `uang kamu tidak cukup untuk melakukan judi sebesar ${count} Money`.trim(),
        m,
      );
  } catch (e) {
    console.log(e);
...
handler.help = ["judi"].map((v) => v + " (Ⓕ)");
handler.tags = ["rpg"];
handler.command = ["judi"];
