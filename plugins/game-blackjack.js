    ...
    switch (aksi) {
      ...
      case "start":
        if (conn.blackjack[m.chat]) {
          await conn.reply(
            m.chat,
            `*Sesi blackjack sudah berlangsung.* Gunakan *${usedPrefix + command} end* untuk keluar dari sesi.`,
            m,
          );
        } else {
      ...
      default:
        await conn.reply(
          m.chat,
          `*Perintah tidak valid.*\nGunakan *${usedPrefix + command} start* untuk memulai sesi blackjack.`,
          m,
        );
        break;
    }
...
handler.help = ["blackjack"].map((v) => v + " (Ⓕ)");
