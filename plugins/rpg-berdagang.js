let handler = async (m, { conn, text }) => {
  ...
  if (new Date() - global.db.data.users[m.sender].lastdagang > 28800000) {
    if (4999 > users[who].money)
      throw "Target tidak cukup modal untuk berdagang Rp5000";
    if (4999 > users[m.sender].money)
      throw "kamu tidak memiliki cukup modal untuk berdagang Rp5000";
...
handler.help = ["berdagang"].map((v) => v + " (Ⓕ)");
handler.tags = ["rpg"];
