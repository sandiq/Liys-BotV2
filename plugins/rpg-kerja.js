  ...
  if (/kerjadulu|kerja|work/i.test(command)) {
    switch (type) {
      case "ojek":
        ...
        m.reply(
          `Kamu Sudah Mengantarkan *${penumpang}*\nDan Mendapatkan Uang Senilai *$.${uangm}*`,
        );
        break;
      case "pedagang":
        ...
        global.db.data.users[m.sender].lastkerja = new Date() * 1;
        m.reply(
          `Ada Pembeli Yang Membeli *${dagangan}*\nDan Mendapatkan Uang Senilai *$.${duit} *`,
        );
        if (pecat > 1) {
      ...
      case "dokter":
        ...
        global.db.data.users[m.sender].lastkerja = new Date() * 1;
        m.reply(
          `Kamu Menyembuhkan Pasien *${pasien}*\nDan Mendapatkan Uang Senilai *$.${duitm}*`,
        );
        break;
      case "petani":
        ...
        global.db.data.users[m.sender].lastkerja = new Date() * 1;
        m.reply(
          `${panen} Sudah Panen Dan Menjualnya\nHasil Menjual Mendapatkan Uang Senilai *$.${duitd}*`,
        );
        break;
      case "montir":
      ...
      case "kuli":
        ...
        global.db.data.users[m.sender].lastkerja = new Date() * 1;
        m.reply(
          `Kamu Baru Saja Selesai ${rumah}\nDan Mendapatkan Uang Senilai *$.${duitk}*`,
        );
        break;
      default:
...
handler.help = ["kerja", "work"].map((v) => v + " (Ⓕ)");
handler.tags = ["rpg"];
